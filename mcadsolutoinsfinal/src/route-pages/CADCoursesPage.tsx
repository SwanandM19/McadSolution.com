...
interface CADCoursesPageProps {
  onNavigate?: (page: string) => void;
}

export function CADCoursesPage({ onNavigate }: CADCoursesPageProps) {
  const courses = [
  {
    id: 'catia',
    title: 'CATIA V5',
            ...
            {courses.map((course) => (
              ...
              >
                <div className="h-full overflow-clip relative rounded-[inherit] p-[25px]">
                  ...
                  <button
                    className="w-full rounded-full text-white font-['Geist:Regular',sans-serif] text-[14px] py-[10px]"
                    style={{
                      background:
                        'linear-gradient(90deg, #10162F 0%, #FBBF24 100%)',
                    }}
                    onClick={() => onNavigate?.(getCoursePage(course.id))}
                  >
                    Enroll Now
                  </button>
                </div>
